...
var help = [
  ...
  '  --spawnMode      --spawnMode will start script in spawn mode',
  '  --forkCount      --forkCount how many worker should start in fork mode',
  ...
  '  --killTTL        Gracefully shutdown wait time',
...
var getOptions = cli.getOptions = function (file) {
  var options = {},
    ...
    configKeys = [
      ...
      'killTree', 'killSignal', 'id', 'spawnMode', 'forkCount', 'killTTL'
    ],
  ...
  options.watchIgnorePatterns = Array.isArray(options.watchIgnore) ? options.watchIgnore : [options.watchIgnore];
  ...
  if (options.forkCount === 0) {
    options.forkCount = cpus;
  }

  if (!options.forkCount && !options.spawnMode) {
    forever.log.warn('--forkCount not set. Defaulting to: 1');
    options.forkCount = 1;
  }
