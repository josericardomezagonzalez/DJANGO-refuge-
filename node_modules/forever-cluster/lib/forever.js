...
forever.kill = require('forever-monitor-cluster').kill;
forever.checkProcess = require('forever-monitor-cluster').checkProcess;
forever.root = process.env.FOREVER_ROOT || path.join(process.env.HOME || process.env.USERPROFILE || '/root', '.forever');
forever.config = new nconf.File({
  file: path.join(forever.root, 'config.json')
});
forever.Forever = forever.Monitor = require('forever-monitor-cluster').Monitor;
...
function stopOrRestart(action, event, format, target) {
  ...
  function sendAction(proc, next) {
    ...
    function onMessage(data) {
      ...
      //
      return type === 'error' && /is not running/.test(message) ? next(new Error(message)) : next(null, data);
    }
  ...
  getAllProcesses(function (err, processes) {
    ...
    if (target !== undefined && target !== null) {
      ...
      procs = procs || forever.findById(target, processes) || forever.findByIndex(target, processes) || forever.findByUid(target, processes) || forever.findByPid(target, processes);
    }
    else {
      procs = processes;
    }
...
forever.load = function (options) {
  ...
  forever.config = new nconf.File({
    file: path.join(options.root, 'config.json')
  });
  ...
  if (!options.columns) {
    options.columns = [
      'uid', 'command', 'script', 'forever', 'pid', 'restarts', 'uptime'
    ];
  }
  ...
  // //
  forever.out.transports.console.timestamp = forever.config.get('timestamp') !== 'false';
  //
...
forever.stat = function (logFile, script, callback) {
  ...
  fs.stat(script, function (err, stats) {
    ...
    return logAppend ? callback(null) : fs.stat(logFile, function (err, stats) {
      return !err ? callback(new Error('log file ' + logFile + ' exists. Use the -a or --append option to append log.')) : callback(null);
    });
  });
};
...
forever.tail = function (target, options, callback) {
  ...
  function tailProcess(procs, next) {
    ...
    tail.stdio[1].on('data', function (data) {
      ...
      chunk.forEach(function (logs) {
        ...
        proc = file.length ? map[file[0].split(' ')[1]] : map[procs[0].logFile];

        lines = count !== 1 ? logs.slice(1) : logs;
  ...
  getAllProcesses(function (err, processes) {
    ...
    var procs = forever.findByIndex(target, processes) || forever.findByScript(target, processes);
...
forever.findByUid = function (script, processes) {
  var procs = !processes ? null : processes.filter(function (p) {
    return p.uid === script;
  });
...
forever.format = function (format, procs) {
  ...
  if (format) {
    ...
    //
    procs.forEach(function (proc) {
      proc.pids.forEach(function (singleProc, singleProcIndex) {
        rows.push(mapColumns('[' + index + ']', function (column) {
          return forever.columns[column] ? forever.columns[column].get({
            ctime: proc.ctime[singleProcIndex + ''],
            command: proc.command,
            file: proc.file,
            foreverPid: proc.foreverPid,
            logFile: proc.logFile,
            args: proc.args,
            pid: proc.pids[singleProcIndex],
            uid: proc.uid,
            id: proc.id,
            spawnWith: proc.spawnWith,
            running: proc.running,
            restarts: proc.restarts[singleProcIndex + ''],
    ...
    formatted = cliff.stringifyRows(rows, mapColumns('white', function (column) {
      return forever.columns[column] ? forever.columns[column].color : 'white';
    }));
  }
...
forever.cleanUp = function (cleanLogs, allowManager) {
  ...
  getAllProcesses(function (err, processes) {
    ...
    function cleanProcess(proc, done) {
      ...
      else if (!proc.child && !proc.manager || (!proc.child && proc.manager && allowManager) || proc.dead) {
    ...
    if (processes && processes.length > 0) {
      (function cleanBatch(batch) {
        async.forEach(batch, checkProcess, function () {
          return processes.length > 0 ? cleanBatch(processes.splice(0, 10)) : emitter.emit('cleanUp');
        });
...
forever.logFilePath = function (logFile, uid) {
  return logFile && (logFile[0] === '/' || logFile[1] === ':') ? logFile : path.join(forever.config.get('root'), logFile || (uid || 'forever') + '.log');
};
...
forever.pidFilePath = function (pidFile) {
  return pidFile && (pidFile[0] === '/' || pidFile[1] === ':') ? pidFile : path.join(forever.config.get('pidPath'), pidFile);
};
...
forever.logEvents = function (monitor) {
  ...
  monitor.on('watch:restart', function (info) {
    forever.out.info('restarting script because ' + info.file + ' changed');
  });

  monitor.on('worker:restart', function (pid, index) {
    forever.out.info('Script ' + monitor.data.file + ' pid:' + pid + ' restarted by attempt #' + monitor.times[index] + (monitor.isSpawn ? '' : ' worker: ' + index));
  });

  monitor.on('worker:start', function (pid, index) {
    forever.out.info('Script ' + monitor.data.file + ' pid:' + pid + (monitor.isSpawn ? ' start' : ' worker ' + index + ' started'));
  });

  monitor.on('worker:stop', function (pid, index) {
    forever.out.info('Script ' + monitor.data.file + ' pid:' + pid + (monitor.isSpawn ? ' stopped' : ' worker ' + index + ' stopped'));
  });

  monitor.on('stop', function () {
    forever.out.info('Script ' + monitor.data.file + ' stopped');
  });

  monitor.on('error', function (error) {
    forever.out.info('Monitor ' + monitor.data.file + ' encounter an error ' + error.message + '\n' + error.stack, ' trying to restart');
    if (error.code === 'EFSEND') {
      process.exit(1);
    } else if (error.code === 'ESEND') {
      setTimeout(() => {
        monitor.restart();
      }, 1000);
    }
  ...
  monitor.on('exit:code', function (pid, index, code, signal) {
    var info = 'Forever detected pid: ' + pid + ' script: ' + monitor.data.file + ' worker: ' + index;
    if (code !== null && code !== undefined) {
      forever.out.error(info + ' exited with code: ' + code);
    }
    else {
      forever.out.info(info + ' was killed by signal: ' + signal);
    }
  });
};
